...
export class LoginPage implements ViewWillEnter, ViewDidEnter, ViewWillLeave, OnDestroy {
  ...
  // Profile-completion state for the phone-only sign-up path.
  phoneInfoName = '';
  phoneInfoEmail = '';
  phoneInfoPhotoFile: File | null = null;
  phoneInfoPhotoPreview: string | null = null;

  // OTP resend countdown — shared by both phone-only and Google flows.
  resendSecondsLeft = 0;
  ...
  async verifyOtp(): Promise<void> {
    ...
    try {
      ...
      // First-time phone sign-up: collect name + email + optional photo before dashboard.
  ...
  // ---------- phone-info (profile completion after first phone OTP) ----------
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        this.api.postForm<{ user: AuthUser }>('/me/profile', fd).subscribe({
          next: (res) => {
            // Merge updated fields into the locally-cached user.
  ...
  async resendGoogleOtp(): Promise<void> {
    if (this.resendSecondsLeft > 0 || this.loading) return;
    // Re-send to the same phone the user already entered.
    const normalized = normalizePhoneToE164(this.googlePhone);
