...
/**
 * Dev-only location override. Reads/writes a single localStorage entry that
 * GeolocationService consults before hitting real GPS. Gated on
 * `environment.production` — in prod builds isEnabled() returns false and the
 * service short-circuits, so no override path can ever run for real users.
 */
@Injectable({ providedIn: 'root' })
export class DevLocationService {
  private readonly subject = new BehaviorSubject<DevLocationOverride | null>(this.read());

  /** True only in non-production builds. Production short-circuits everywhere. */
  isEnabled(): boolean {
    return !environment.production;
  }

  /** Current override (or null if real GPS should be used). */
  get(): DevLocationOverride | null {
    return this.isEnabled() ? this.subject.value : null;
  }

  /** Observable stream so the modal and watchers can react to changes. */
  ...
  set(override: DevLocationOverride): void {
    ...
    try {
      ...
      /* private mode etc — keep in-memory at least */
    }
