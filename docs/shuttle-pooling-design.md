...
7. 🔨 **Multi-passenger ride** (incl. 5B boarding) — **backend DONE & tested**: `ShuttleDriverService` (manifest / board / drop / allRidersFinished) + `ShuttleDriverController` + driver routes (`GET /shuttle/journeys/{journey}/manifest`, `POST /shuttle/bookings/{booking}/board|drop`) + live `ShuttleManifestUpdated` broadcast on the trip tracking channel. Rider lifecycle CONFIRMED→BOARDED→DROPPED; `settleShuttle` fixed to settle `DROPPED` (was filtering a dead `COMPLETED`). Boarding mode: `driver_only` tap built in. Tests: `ShuttleDriverManifestTest` (6). **Driver live pool screen — DONE.** `driver-mobile` `rides.page.ts` now loads the pool via `GET /shuttle/trips/{trip}/manifest` (trip-based endpoint added so the app needs only the trip id), renders a "Pool riders" card (aboard/remaining + per-rider Board / Drop off buttons + status), and refreshes live off `ShuttleManifestUpdated` (bound in `RealtimeService.subscribeTripStatus`, same tracking channel). TS + AOT build clean; **visual device check is the user's**. Tests: `ShuttleDriverManifestTest` (7, incl. trip endpoint).

**Boarding modes (5B) — DONE.** `shuttle_boarding_confirmation_mode` city setting (**driver_only / customer_otp / driver_customer** — QR removed from all ride types per the user). Admin select in City Settings → Shuttle. `ShuttleBoardingOtpService` mirrors Fixed — system-generated 4-digit code, hashed with TTL + resend-cooldown + attempt-lock, shown on the rider's OWN screen (in-app, **no MSG91/SMS**). `ShuttleDriverService::board()` enforces the mode (driver_only = tap; else verify code); `POST /shuttle/bookings/{booking}/boarding-otp` generates it; the rider's booking exposes `boarding_code` + `boarding_mode`. Driver app: mode-aware board flow (generate → prompt for code → verify). Customer app: prominent boarding-code card on the booking. Tests: `ShuttleDriverManifestTest` (9, incl. otp require + reject-wrong).

**Full-screen boarding-code prompt (all 3 ride types) — DONE.** Shared standalone `BoardingCodePromptComponent` (customer-mobile `shared/`) — full-screen overlay, boxed-digit code (Fixed's design), safety warning, "Got it" dismiss; auto-pops when a code first arrives, re-pops on a fresh code. Wired into `trip-active` (private `start_otp` **and** shuttle via new `GET /shuttle/trips/{trip}/my-booking`) and `fixed-ride-active` (`booking.boarding_code`). **Instant:** a new `ShuttleBoardingCodeReady` broadcast (fired from `ShuttleBoardingOtpService::send()`, mirrors `StartOtpReady`) pops the prompt with no delay — the customer realtime binds it to the same mode-aware `onStartOtp` fetch; a 15s poll is only a socket-down fallback. Customer-mobile TS + AOT build clean; **on-device visual check is the user's**. (Inline code cards remain as a fallback reference.)

**QR removed (all ride types):** `qr_scan` dropped from the boarding-mode option lists in Private/Shuttle/Fixed — validations (`AdminCitySettingsController` ×2, `AdminFixedRoutesController`, `FixedRouteService` whitelist), frontend type unions (`city-settings`, `fixed-routes`), the Shuttle admin select option, and the driver-app prompt. Remaining modes everywhere: driver_only / customer_otp / driver_customer. (Private never had a boarding-mode QR — it uses `start_otp`.)
