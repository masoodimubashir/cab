...
**Only remaining sub-piece: (b)** OTP / QR boarding modes — boarding is `driver_only` (tap) today; `customer_otp` / `qr_scan` need a per-booking code store + a `shuttle_boarding_confirmation_mode` city setting (follow-on).
