...
4. **Phase 4 — Kill cash + rebuild admin payout/refund views into monitors.** ~1 wk.
   ...
   - ⏳ **Remaining:** driver-app (Ionic) cash-UI removal + earnings-net display; admin ledger/reconciliation view + refunds-as-disputes scoping (the `payCash` route itself is retired in the final cash-dead cleanup, not while the flag can still be off).
5. **Phase 5 — Roll Fixed & Shuttle onto the shared engine; ledger/reconciliation hardening.** ~1 wk.
   ...
   - ⏳ **Remaining:** admin visibility of booking-mode payments in the ledger monitor (they show today, but aren't grouped per departure).
