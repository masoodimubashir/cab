...
3. **Phase 3 — Auto-refund rulebook + transfer reversal + held-earnings release** (`AutoRefundService`, `HeldEarningsService`). ~1–1.5 wks.
   ...
   - ⏳ **Remaining:** `refund.processed`/`refund.failed` webhook handling + sweeper retry of stuck/failed refunds (pairs with the deferred Phase 2 webhook work). *(Fixed/Shuttle seat-release refunds R6/R7 — done in Phase 5.)*
...
5. **Phase 5 — Roll Fixed & Shuttle onto the shared engine; ledger/reconciliation hardening.** ~1 wk.
   ...
   - ⏳ **Remaining:** admin visibility of booking-mode payments in the ledger monitor (they show today, but aren't grouped per departure); the Fixed "departure expired before dispatch" bulk refund still runs on the legacy path.
