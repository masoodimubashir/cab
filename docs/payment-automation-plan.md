...
| Fixed/Shuttle cancelled **before a driver is committed** | **Full auto refund** — nothing was promised to anyone yet |
| Fixed/Shuttle cancelled **once a driver is committed**, or no-show | **No refund** — the driver took the journey on the strength of the seats sold |
...
2. **Phase 2 — Shared payment engine + auto-split + prepay-at-booking (Private first)** (`PaymentsController` rework, `PaymentSplitService`, `RazorpayService` transfers, ledger). ~2–3 wks.
   ...
   - ✅ **Prepay-at-booking for Private + Route webhooks + payout sweeper** (done after Phase 5, before Phase 6). A private ride is now payable the moment the fare is agreed (`CONFIRMED`), captured with `settlement_mode = 'booking'` and split at completion — the same deferred seam Fixed/Shuttle use, because the ride hasn't happened yet and the driver isn't owed anything until it has. `payments.trip_id` already allows several rows per trip, so a ride that ran longer than quoted bills only the shortfall, and one that came in cheaper is auto-refunded the difference *before* the split, so the driver is paid on the fare that stands. The cancel fee is stamped on the trip at prepay time, so the §5 rulebook works "from the moment of booking" as designed. New `PayoutReconciliationService` handles `transfer.processed` / `transfer.failed` (re-queues the driver's share with no second ledger allocation) and `account.activated` / `account.suspended` (verifies or rejects, and releases everything parked); `refund.processed`/`refund.failed` now settle the engine's own claim via `payments.refund_id` and mirror onto the booking row. New `payments:reconcile-payouts` (every 10 min) chases stuck transfers, releasable holds, pending KYC and failed refunds. Customer app pays off a server-provided `payment_due` rather than the trip status. Tests: `PrivatePrepaymentTest` (13), `PayoutWebhookTest` (10), `PayoutSweeperTest` (11) — all green.
   - ⏳ **Remaining (needs live Route):** passing the split payload at order time so Razorpay does the division itself, rather than us transferring after capture.
3. **Phase 3 — Auto-refund rulebook + transfer reversal + held-earnings release** (`AutoRefundService`, `HeldEarningsService`). ~1–1.5 wks.
   - ✅ **Auto-refund engine built + tested** (flag-gated, default off): `AutoRefundService` implements the §5 rulebook (`decide()` pure math + `refundForCancellation()` execution) — refund = fare − commission cancel fee for a customer pre-pickup cancel, full refund when it's not the customer's fault, no refund after arrival; the driver's whole share is always clawed back (`RazorpayService::reverseTransfer` for a live transfer, `HeldEarningsService::reverseHeldForPayment` for a still-held share), the operator keeps only the cancel fee. Idempotent claim (row-lock + `payments.refund_status`) collapses a double-cancel race and keeps a failed refund retryable. New `payments` refund columns; `LedgerService::tripBalance` now reports `driver_net`/`operator_net` and reconciles after refunds. Wired into the solo `CANCELLED` transition (`TripStateMachineService`). Tests: `AutoRefundDecisionTest` (unit, rulebook rows) + `AutoRefundEngineTest` (R1–R5, R9, R10 partial, R11 fail + retry, R12 double-cancel, K4 held reversal) — all green, each asserting the ledger still reconciles.
   - ✅ **Refund webhooks + sweeper retry — done** alongside the Phase 2 webhook work above. *(Fixed/Shuttle seat-release refunds R6/R7 — done in Phase 5.)*
4. **Phase 4 — Kill cash + rebuild admin payout/refund views into monitors.** ~1 wk.
   - ✅ **Cash killed + payout/ledger monitors built + tested** (flag-gated): the single `PaymentModeService::allowedForTrip` gate returns online-only once the engine is on, so both the customer screen and the `payCash` server guard drop cash together (legacy cash still works while the flag is off). New `PayoutMonitorService` + `Admin\PayoutMonitorController`: `GET /admin/payouts/monitor` (driver transfers bucketed paid/held/failed/reversed with Route references) and `GET /admin/ledger` (every movement + per-trip reconciliation), both under `permission:finance`. Admin `drivers-payouts.tab.ts` rebuilt from a manual worklist into a status monitor (legacy wallet "owed" kept as a secondary card during migration). Tests: `CashKillTest` + `PayoutMonitorTest` — green; admin `ng build` clean.
   - ✅ **Driver app + admin money screens — done.** Driver: the trip screen reads a server-computed `driver_payout` (fare, commission, what they earn, and `collect_cash` — false whenever the engine is on) instead of inferring from `payment_method`, and says plainly not to take cash; the earnings page gains a payouts card off a new `payout` block on `/drivers/me/earnings` showing received / on its way / waiting to be released, distinguishing money stuck behind KYC (chase them) from a bounced transfer (already retrying). Admin: new **Finance → Money Ledger** page over `/admin/ledger` — every movement, per-trip reconciliation, one-click "only trips that don't balance", scoped by trip; the payout monitor gains a **"drivers waiting to be paid"** roll-up (`owed`) answering who is owed and why; the refunds register re-frames itself as an audit trail with an exceptions banner once `auto_refunds` is on. Tests: `DriverPayoutVisibilityTest` (10) + `AdminMoneyScreensTest` (11) — green; both app builds clean.
   - ✅ **Ride-end extras + payout alerts — done.** The driver declares what the meter can't see as they finish (`extra_toll_amount` / `extra_waiting_amount` on `driver-progress`); both raise the negotiated floor so they're actually billable, land in their own columns, and the toll stays out of the commission base. The completion response returns `balance_due` and `driver_net`, so the summary sheet shows the driver what *they* earn rather than the fare and flags anything left for the rider to settle online. Held or bounced earnings now notify the driver (`driver_earnings_held` / `driver_payout_failed`) with the reason and what to do, instead of only reaching a log file.
   - ⏳ **Remaining:** the `payCash` route itself is retired in the final cash-dead cleanup, not while the flag can still be off.
5. **Phase 5 — Roll Fixed & Shuttle onto the shared engine; ledger/reconciliation hardening.** ~1 wk.
   - ✅ **Fixed & Shuttle prepayments moved onto the shared engine + tested** (flag-gated). The timing problem is the whole phase: those customers prepay **at booking**, onto their own booking rows (`SeatReservation` / `ShuttlePassengerBooking`) — for Fixed there is no trip yet at all, and for Shuttle no driver — so the split cannot run at capture the way the solo path's does. New `BookingPaymentService` is the seam: `recordCapture()` mirrors each prepayment as a `Payment` row with `settlement_mode = 'booking'` (idempotent on the Razorpay payment id), `linkDepartureBookings()` attaches those rows to the trip the moment a fixed departure is dispatched or a driver starts it, and `settleTrip()` splits them all at completion via the new `PaymentSplitService::settleBookingPayment()` — figures passed per booking, because one departure/journey carries many independent fares on a single trip. Settlement hangs off `CommissionSettlementService::settle()`, the one choke point every completion path already goes through. `payments.trip_id` lost its `unique` and became nullable to allow both (migration). Seat-release refunds (R6/R7) land through `AutoRefundService::refundBookingCancellation()`, wired into `FixedRefundService` (customer cancel / system cancel / no-show) and `ShuttleRefundService` (cancel / journey cancel / no-show): a cancel before completion means the driver was never paid, so it's a clean full refund with no transfer reversal; a no-show or too-late cancel forfeits the fare to the operator, recorded on the ledger so the trip still reconciles. A Razorpay refund failure falls back to the legacy manual "APPROVED = owed" register. The legacy shared-journey wallet credit is suppressed when the engine is on, so Route + wallet can't double-pay. Tests: `BookingSettlementPhase5Test` (15, full HTTP Fixed flow) + `ShuttleSettlementPhase5Test` (7) — mirror/link/split, many bookings on one trip, held share for an unverified driver, R6/R7, double-cancel idempotency, failed refund, and both engine-off no-ops — all green, each asserting the ledger reconciles. Whole payment suite: 54 green.
   - ⏳ **Remaining:** booking-mode payments show in the ledger but aren't grouped per departure — a Fixed vehicle's seats read as separate rows on one trip rather than as one journey.
...
| R6 | Fixed/Shuttle cancelled before a driver is committed | Full auto refund; seat returns to inventory |
| R7 | Fixed/Shuttle cancelled after a driver is committed, or no-show | No refund; fare forfeited to the operator |
